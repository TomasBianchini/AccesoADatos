...
public class DataPersona {
	...
	public LinkedList<Persona> getByApellido(Persona per){
		...
		try {
			...
			pstmt.setString(1,per.getApellido());
	...
	public void edit(Persona p) {
		PreparedStatement pstmt = null;
		try {
			pstmt = DbConnector.getInstancia().getConn().prepareStatement("UPDATE persona SET nombre=?,apellido=?,email=?,password =?,tel=?, habilitado=? WHERE tipo_doc=? AND nro_doc=? ");
			...
			pstmt.setBoolean(6, p.isHabilitado());
			pstmt.setString(7, p.getDocumento().getTipo());
			pstmt.setString(8, p.getDocumento().getNro());
			pstmt.executeUpdate();	
		}  catch (SQLException e) {
            e.printStackTrace();
		} finally {
	...
	public void delete(Persona p) {
		PreparedStatement pstmt = null;
		DataRol_personas dr_p = new DataRol_personas();
		try {
			pstmt = DbConnector.getInstancia().getConn().prepareStatement(
